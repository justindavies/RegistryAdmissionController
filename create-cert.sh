...
[ -z ${service} ] && service=registry-admission-service
[ -z ${secret} ] && secret=registry-admission-certs
...
openssl req -new -key certs/server-key.pem -subj "/CN=${service}.${namespace}.svc" -out certs/server.csr -config certs/csr.conf
